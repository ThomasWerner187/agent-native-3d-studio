...
const fixture = {
  ...
  human_actions: [
    { clip: 'human_pond', source_seconds: at('human_pond', 5), kind: 'place', id: 'pond' },
    { clip: 'human_cabin', source_seconds: at('human_cabin', 2), kind: 'place', id: 'cabin' },
    { clip: 'human_move', source_seconds: at('human_move', 6), kind: 'move', id: 'tree_0' },
  ],
  events: [
    event('human_cabin', 3, 'help', {}, { guide: 'Synthetic test guide' }), query('human_cabin', 16, anchors),
    event('agent_forest', 3, 'scatter', { type: 'tree', count: 30 }, { added: 30, live_added: 30, exact_count: true, ids: trees.map(item => item.id), preserved_ids: ['pond', 'cabin'] }),
    query('agent_forest', 20, [...anchors, ...trees]),
    event('agent_details', 3, 'scatter', { type: 'rock', count: 6 }, { added: 6 }), event('agent_details', 6, 'scatter', { type: 'lamp', count: 4 }, { added: 4 }),
    query('agent_details', 14, details), query('agent_readback', 13, changed),
    event('agent_readback', 14, 'describe_scene', {}, { selected_id: 'tree_0', human_edits: [{ id: 'tree_0' }], recent_changes: [{ id: 'tree_0', actor: 'human' }] }),
    event('atmosphere', 2, 'set_lighting', { preset: 'moonlit' }, {}), event('atmosphere', 5, 'set_camera_motion', { action: 'start' }, { camera_motion: { status: 'running' } }),
    event('atmosphere', 13, 'describe_scene', {}, { camera_motion: { status: 'running' }, music: { playing: true } }), query('atmosphere', 17, changed),
  ],
};
