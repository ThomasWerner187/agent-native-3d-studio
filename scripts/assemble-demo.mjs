...
function probe(file) {
  return JSON.parse(run(ffprobe, ['-v', 'error', '-show_entries', 'format=duration:stream=codec_name,codec_type,width,height,avg_frame_rate', '-of', 'json', file], 30_000));
}
...
function validateTimeline() {
  ...
  for (let i = 0; i < timeline.segments.length; i++) {
    ...
    if (!Number.isFinite(actual) || actual + 0.5 > segment.duration) throw new Error(`Voiceover ${segment.id} does not fit its shot.`);
...
validateTimeline();
if (planning) {
  console.log(JSON.stringify({ output, duration: timeline.duration, shots, music, voice: timeline.voice, transcript: join(output, 'native-capture.json') }, null, 2));
  process.exit(0);
}
...
for (const segment of timeline.segments) {
  ...
  chunks.chunks.forEach((chunk, index) => {
    const start = segment.start + 0.5 + segment.audio_duration * chunk.from / chunks.words;
    const end = segment.start + 0.5 + segment.audio_duration * chunk.to / chunks.words;
...
if (checking) {
  ...
  const mix = `[${voiceIndex}:a]loudnorm=I=-16:TP=-1.5:LRA=9,aresample=48000,aformat=channel_layouts=stereo[voice];[${musicIndex}:a]loudnorm=I=-25:TP=-4:LRA=11,aresample=48000,aformat=channel_layouts=stereo,volume=0.45[music];[voice][music]amix=inputs=2:normalize=0,alimiter=limit=0.96:level=false[mix]`;
...
const audio = timeline.segments.map((segment, i) => `[${i + 1}:a]loudnorm=I=-16:TP=-1.5:LRA=9,aresample=48000,aformat=channel_layouts=stereo,adelay=${Math.round((segment.start + 0.5) * 1000)}:all=1[voice${i}]`);
audio.push(`[9:a]atrim=duration=135,asetpts=PTS-STARTPTS,loudnorm=I=-25:TP=-4:LRA=11,aresample=48000,aformat=channel_layouts=stereo,volume=0.45,afade=t=in:st=0:d=2,afade=t=out:st=131:d=4[music]`);
audio.push(`${timeline.segments.map((_, i) => `[voice${i}]`).join('')}[music]amix=inputs=9:normalize=0:duration=longest,alimiter=limit=0.96:level=false,atrim=duration=135,afade=t=out:st=134:d=1[mix]`);
...
console.log('Mixing normalized English narration and a quiet lofi bed.');
run(ffmpeg, args);
...
if (Math.abs(Number(final.format.duration) - 135) > 0.1 || video?.width !== width || video?.height !== height || video?.codec_name !== 'h264' || sound?.codec_name !== 'aac') {
...
writeFileSync(join(output, 'export-metadata.json'), JSON.stringify({ generated_at: new Date().toISOString(), master, format: final, shots, voice: timeline.voice, captions: cues.length, native_events: events.length, music, source: 'native-capture.json', caption_timing: 'Approximate word timing from measured per-segment narration duration.' }, null, 2));
writeFileSync(join(output, 'README.md'), `# Submission film\n\n- Master: [submission-demo.mp4](submission-demo.mp4), 135 seconds, 1280 × 720, H.264 CRF 18 video / AAC audio.\n- [Captions](submission-demo.srt) use approximate word timing from the measured voiceover tracks.\n- [Contact sheet](submission-contact-sheet.jpg) and [poster](submission-poster.jpg) are inspection artifacts.\n- Native tool evidence comes from [native-capture.json](native-capture.json). Cards show selected recorded arguments and results; they are editorial excerpts, not recreated chat.\n- English narration is synthesized locally using the macOS ${timeline.voice} voice. Eight source AIFF tracks are retained with [timeline.json](timeline.json).\n- Music: Aurora Drift, created and supplied by Thomas Werner using Suno, mixed quietly beneath the narration.\n- Export is 30 FPS; browser capture timing is preserved. The 30 FPS file is not a claim that the original screencast captured every displayed frame.\n- The app footage is edited into the narrated sequence. No upload or Devpost submission is performed by this script.\n\nRebuild: \`node scripts/assemble-demo.mjs\` from the repository root. The script waits for complete raw clips and native evidence. \`--plan\` validates the timeline; \`--check\` checks local text rendering and ffmpeg compositing without waiting for captures.\n`);
