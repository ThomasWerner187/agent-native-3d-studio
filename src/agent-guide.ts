...
export const AGENT_PLAYBOOK = [
  'LOFI: compose_lofi_scene{mood:"moonlit"|"golden_hour",build_seconds:32,seed:42,camera:"cinematic"|"orbit",music:true} replaces the scene (undoable), progressively builds a cabin, pond, forest and lanterns, then fades in music and loops the camera. Returns immediately with session_id; describe_scene reports actual progress, camera_motion and audio status. control_lofi{action:"pause"|"resume"|"stop"}. set_camera_motion{action:"start",mode:"orbit",loop_seconds:240} works on any scene. Human input pauses motion; explicitly resume. Music may need a user gesture. The local Create button invokes the same handler, not an AI model. Recording is not implemented.',
