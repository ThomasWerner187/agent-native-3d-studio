...
- **The reliability contract:** mutating tools `await` their animation and report the *live rendered* values plus `scene_version` and `operation_id` afterwards, so an agent's follow-up read always matches its last write. Human interruption is reported as `applied: false` with live state — never as a fake success.
